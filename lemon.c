...
static void handle_h_option(char *z) {
    report_name = (char *) malloc( lemonStrlen(z)+1 );
    if(report_name == 0) {
        memory_error();
    }
...
int main(int argc, char **argv)
{
  ...
  static struct s_options options[] = {
    ...
    {OPT_FSTR, "u", (char*)&handle_h_option, "Specify the report header filename."},
...
PRIVATE char *file_makename(struct lemon *lemp, const char *suffix)
{
  ...
  if(NULL != report_name && 0 == strcmp(suffix, ".h")) {
      name = (char*)malloc( lemonStrlen(report_name) + 7);
      src = report_name;
  }
  else {
